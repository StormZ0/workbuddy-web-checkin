#!/bin/bash
# web-checkin.sh —— WorkBuddy 网页版每日签到（幂等 v2）
# 安全设计：当天短路（≤1次接口调用）| 随机延迟 | 异常熔断 | 日志无凭据
set -u
source "$(dirname "$0")/common.sh"
ensure_runtime

MARK="$STATE_DIR/.done_$(date +%F)"
FAILS="$STATE_DIR/.fail_streak"
...
if [[ "$LOC" == *"/login"* || -z "$LOC" ]]; then
  ...
  echo "RESULT: NEED_RELOGIN | 登录态失效，请运行 scripts/first-login.sh 重新扫码"
  exit 2
fi
